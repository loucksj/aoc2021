...
class Caves:
    ...
    def count_paths(self, boost=False, cave=None, visited=None) -> int:
        if not cave:
            cave = self.get_cave('start')
        if cave.name == 'end':
            return 1
        visited = [] if not visited else visited.copy()
        count = 0
        visited.append(cave)
        for to_cave in cave.caves:
            ...
            if to_cave in visited and not to_cave.is_big and boost:
                count += self.count_paths(False, to_cave, visited)
            if to_cave not in visited or to_cave.is_big:
                count += self.count_paths(boost, to_cave, visited)
        return count
